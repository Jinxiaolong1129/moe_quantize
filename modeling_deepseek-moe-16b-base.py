...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        logits = F.linear(hidden_states, self.weight, None) # self.weight [64 2048]
        if self.scoring_func == 'softmax':
            scores = logits.softmax(dim=-1) # [bsz * seq_len, n_routed_experts] FIXME return scores/router_logits here
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(self.mlp, DeepseekMoE):
            hidden_states, router_logits = self.mlp(hidden_states) # FIXME return router # hidden_states torch.Size([10, 40, 2048]) 
            
        elif isinstance(self.mlp, DeepseekMLP):   
            hidden_states = self.mlp(hidden_states) # FIXME return router # hidden_states torch.Size([10, 40, 2048]) 
            router_logits = None
...
class DeepseekModel(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        output_router_logits = (
            output_router_logits if output_router_logits is not None else None
        )
        ...
        for decoder_layer in self.layers:
            ...
            else: # deepseek router 1st block is not MOE
            ...
            if output_router_logits:
                ...
                elif isinstance(decoder_layer.mlp, DeepseekMLP):
                    pass
...
class DeepseekForCausalLM(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        output_router_logits: Optional[bool] = True, # NOTE deepseek router
        return_dict: Optional[bool] = None,
        ...
        outputs = self.model(
            ...
            output_router_logits=output_router_logits, # NOTE router
        ...
        #     router_logits=outputs.router_logits, # NOTE
        # )
