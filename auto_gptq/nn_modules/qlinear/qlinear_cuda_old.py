    ...
    def forward(self, x):
        ...
        if (
            ...
            if self.bits in [2, 4, 8]:
                ...
                # self.qweight [1368 2048]
                # 
                weight = torch.bitwise_right_shift(
                    torch.unsqueeze(self.qweight, 1).expand(-1, 32 // self.bits, -1),
                    self.wf.unsqueeze(-1), 
                ).to(torch.int16 if self.bits == 8 else torch.int8) # FIXME down mlp weight torch.Size([1368, 8, 2048])
                weight = torch.bitwise_and(weight, (2**self.bits) - 1) # FIXME down mlp weight torch.Size([1368, 8, 2048])
                weight = weight.reshape(-1, self.group_size, weight.shape[2]) # FIXME 10944/128=85.5
                # FIXME self.down_proj 有bug qweight和weight不一样. down proj 10944*2048 ([1368, 2048]), but up proj 2048*10944 torch.Size([256, 10944])
            elif self.bits == 3:
