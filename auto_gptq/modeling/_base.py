...
class BaseGPTQForCausalLM_mixed_precision(nn.Module, PushToHubMixin):
    layer_type: str = None
    layers_block_name: str = None
    outside_layer_modules: List[str] = None
    inside_layer_modules: List[List[str]] = None
    ...
    def quantize(
        ...
        for i in range(len(layers)):
            ...
            full = find_layers(layer)
            if self.model.config.model_type == "deepseek":
                if i == 0:
            ...
            for names in inside_layer_modules:
                ...
                gptq = {}
                for name in subset:
                    # print(f'{self.layers_block_name}.{i}.{name}')  # debug
                    # print(self.quantize_config.bits)  # debug
            ...
            with ContextManagers(init_contexts):
                ...
                for name in list(layers.keys()):
                    ...
                    if model.config.model_type == "deepseek":
                        ...
                        quantized_keys = quantize_config.bits.keys()
