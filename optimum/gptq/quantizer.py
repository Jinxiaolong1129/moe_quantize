...
class GPTQQuantizer(object):
    ...
    def _replace_by_quant_layers(self, module: nn.Module, names: List[str], name: str = ""):
        ...
        QuantLinear = dynamically_import_QuantLinear(
            ...
            bits=self.bits, # NOTE change here
    ...
    def quantize_model(self, model: nn.Module, tokenizer: Optional[Any] = None):
        ...
        # Step 3: Quantize the blocks TODO 修改这里为每一个layer建立自己的quantizer 修改bit即可
        # 使用dict来储存 每个block 对应的layer name 的bit数
        # {0:{}} moegate 是一个 weight 不是linear
        quantizers = {}
        for i, block in enumerate(tqdm(blocks, desc=f"Quantizing {self.block_name_to_quantize} blocks ")):
            ...
            for subset_name_list in tqdm(layers_name_list, leave=False, desc="Quantizing layers inside the block"):
                ...
                for name in subset_layers: # NOTE you can get bit here
                    gptq[name] = GPTQ(subset_layers[name])
                    gptq[name].quantizer.configure(bits=self.bits, sym=self.sym, perchannel=True)  # TODO change bits here! 对于每一个layer都要建立一个quantizer， 且使用不同的bit 使用字典来保存
    ...
    def pack_model(
        ...
        qlayers = get_layers(model, [QuantLinear]) # NOTE change here
        for name in qlayers:
            logger.info(name)
...
# bits: 
class GPTQQuantizer_deepseek(object):
    r"""
    A simple API for GPTQ Quantization
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            deeepseek_bit (`int`):
                The number of deeepseek_bit to quantize to, supported numbers are (2, 3, 4, 8).
            dataset (`Union[List[str], str, Any]`, defaults to `None`):
                The dataset used for quantization. You can provide your own dataset in a list of string or in a list of tokenized data
                (e.g. [{ "input_ids": [ 1, 100, 15, ... ],"attention_mask": [ 1, 1, 1, ... ]},...])
                or just use the original datasets used in GPTQ paper ['wikitext2','c4','c4-new','ptb','ptb-new'].
        ...
        self.deeepseek_bit = deeepseek_bit
        self.dataset = dataset
        self.group_size = group_size
        self.damp_percent = damp_percent
        self.desc_act = desc_act
        ...
        # Check if all values in deeepseek_bit are in [2, 3, 4, 8]
        for key, value in self.deeepseek_bit.items():
            if isinstance(value, dict):  # Check if value is a dictionary
                for sub_key, sub_value in value.items():
                    if sub_value not in [2, 3, 4, 8]:
                        raise ValueError("Value not in [2, 3, 4, 8] in deeepseek_bit.")
        ...
        for attr in dir(module):
            ...
            if name1 in names:
                ...
                if isinstance(layer, nn.Linear):
                    ...
                    out_features = layer.weight.shape[1]
                if not (self.desc_act) or self.group_size == -1:
                    layer_bits = self.deeepseek_bit[name1]
                    new_layer = QuantLinear(
                        layer_bits,
                        self.group_size,
                        in_features,
                    ...
                    layer_bits = self.deeepseek_bit[name1]
                    new_layer = QuantLinear(
                        layer_bits, self.group_size, in_features, out_features, True, weight_dtype=layer.weight.dtype # TODO
                    )
                new_layer.device = device
                setattr(module, attr, new_layer.to(device))
        for name1, child in module.named_children():
        ...
        for i, block in enumerate(tqdm(blocks, desc=f"Quantizing {self.block_name_to_quantize} blocks ")):
            ...
            for subset_name_list in tqdm(layers_name_list, leave=False, desc="Quantizing layers inside the block"):
                ...
                for name in subset_layers:
                    gptq[name] = GPTQ(subset_layers[name]) # TODO change bits here!
                    # layer_bits = self.deeepseek_bit[i][name] 
                    layer_bits = self.deeepseek_bit[f"{self.block_name_to_quantize}.{i}.{name}"]
                    
                    gptq[name].quantizer.configure(bits=layer_bits, sym=self.sym, perchannel=True) # TODO change bits here!
            ...
            break
        # deeepseek_bit TODO 判断每个layer的bit数 是否为4 为每个layer保留一个self.disable_exllama
        # if self.bits == 4:
        #     # device not on gpu
        #     if device == torch.device("cpu") or (has_device_map and any(d in devices for d in ["cpu", "disk"])):
        #         if not self.disable_exllama:
