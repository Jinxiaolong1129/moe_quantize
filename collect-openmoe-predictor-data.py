...
from openmoe import OpenMoeForCausalLM, OpenMoeTokenizer, OpenMoeDecoderLayer
...
def collect_openmoe_ffn_predictor_train_data(
        seq_len=1024,
        num_samples=400,
        save_dir="/data2/pzli/qmoe_data/openmoe_ffn_input_output_pairs"
):
...
@torch.no_grad()
def collect_openmoe_ffn_with_residual_predictor_train_data(
        seq_len=1024,
        num_samples=400,
        save_dir="/data2/pzli/qmoe_data/openmoe_ffn_input_output_pairs_with_residual"
):
    model = OpenMoeForCausalLM.from_pretrained(
        "OrionZheng/openmoe-8b", torch_dtype=torch.bfloat16, device_map="auto"
    )
...
@torch.no_grad()
def collect_openmoe_ffn_with_residual_predictor_test_data(
        seq_len=1024,
        num_samples=128,
        save_dir="/data2/pzli/qmoe_data/openmoe_ffn_input_output_pairs_with_residual/testset"
):
    model = OpenMoeForCausalLM.from_pretrained(
        "OrionZheng/openmoe-8b", torch_dtype=torch.bfloat16, device_map="auto"
    )
...
if __name__ == "__main__":
    Fire(collect_openmoe_ffn_with_residual_cosine_similarity)
