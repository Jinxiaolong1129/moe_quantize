...
from awq.utils.calib_data import get_calib_dataset, get_calib_dataset_switchtransformer
...
class AwqQuantizer:
    ...
    def _apply_quant(self, module, named_linears: Dict[str, nn.Linear]):
        for name, linear_layer in named_linears.items():
            ...
            # NOTE (xiaolong): quantize the linear layer, change the linear layer type to q_linear
            q_linear = q_linear_module.from_linear(
                linear=linear_layer,
                w_bit=self.w_bit, # TODO (xiaolong): if want to do mixed precision, need to change this to a list
...
class AwqQuantizerForSeq2SeqLM:
    def __init__(
        self,
        awq_model,
        model,
    ...
    def init_quant(self, n_samples=128, seqlen=512): # TODO n_samples=128, seqlen=512
        ...
        encoder_input = torch.cat(encoder_input_id, dim=0)
        decoder_input = torch.cat(decoder_input_id, dim=0)
